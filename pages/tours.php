...
//        if($countryid == 0) echo $countryid; exit();
//        echo $countryid;
...
echo '</form>';
    if(isset($_POST['selcity'])){
        ...
        while ($row=mysqli_fetch_array($result,MYSQLI_NUM)) {
            echo '<tr id="'.$row[1].'">';
            echo '<td>'.$row[2].'</td> <td>'.$row[0].'</td> <td>'.$row[1].'</td> <td>$'.$row[3].'</td><td>'.$row[4].'</td>  <td><a href="http://localhost:82/pages/hotelinfo.php?hotel='.$row[5].'" target="_blank">more info</a></td>';
            echo '</tr>';
        }
echo '</table><br>';
}
